...
def quarterly_pnl_html() -> str:
    ...
    return (f'<div class="ctitle">분기 평가손익 현황<span class="note">장부(기준가) 기준 · 단위 억원</span></div>'
            ...
            + ('<div style="font-size:10px;color:var(--muted);margin-top:10px;line-height:1.6">'
               '※ 본 표의 평가손익은 사무관리사 기준가 기준으로, 펀드·신탁에 보수 지급용으로 유보된 현금까지 자산에 포함하여 산출됩니다. '
               '주식 가치만 반영하는 상단 평가손익(KRW)과는 이 유보금'
               + (f'(현재 약 {("+" if est["pl"]-plKRW >= 0 else "−")}{abs(est["pl"]-plKRW)/1e8:,.1f}억)' if est else '')
               + '만큼 차이가 있습니다.</div>'))
